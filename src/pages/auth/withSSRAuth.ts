  ...
  return async (ctx: GetServerSidePropsContext): Promise<GetServerSidePropsResult<P>> => {
    ...
    if (options) {
      ...
      if (!userHasValidPermissions) {
        // se não tiver permissão para ver a pagina...
        return {
          redirect: {
            // redirect para uma pagina onde todos usuarios (logados) tem permissão de ver
    ...
    try {
      ...
      if (error instanceof AuthTokenError) {
        // se rolar algum erro com authToken durante requisições server-side
        destroyCookie(ctx, '@rw:token')
        destroyCookie(ctx, '@rw:refreshToken')
